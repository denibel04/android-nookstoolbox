...
class VillagerListAdapter(private val context: Context, private val onVillagerClicked: ((Villager) -> Unit)? = null):ListAdapter<Villager,VillagerListAdapter.VillagerViewHolder>(VillagerDiffCallback) {
    inner class VillagerViewHolder(private val binding: VillagerListItemBinding):RecyclerView.ViewHolder(binding.root){
        fun bind(v:Villager) {
    ...
    private object VillagerDiffCallback:DiffUtil.ItemCallback<Villager>(){
        override fun areItemsTheSame(oldItem: Villager, newItem: Villager) = oldItem.name == newItem.name
        override fun areContentsTheSame(oldItem: Villager, newItem: Villager) = oldItem == newItem
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): VillagerViewHolder = VillagerViewHolder(VillagerListItemBinding.inflate(
